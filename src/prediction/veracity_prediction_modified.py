...
def quadruple_to_string(claim, question, answer, bool_explanation=""):
    ...
    #         " [QUESTION] " + question.strip() + " " + answer.strip()]
    return [" [QUESTION] " + question.strip() + " " + answer.strip(),
            "[CLAIM] " + claim.strip()]
...
if __name__ == "__main__":
    ...
    trained_model = CrossEncoder("output/training_averitec_nli-2024-07-08_16-17-16_epoch_4", tokenizer_args={"use_fast": False})
    # trained_model = CrossEncoder("cross-encoder/nli-deberta-v3-large", tokenizer_args={"use_fast": False})
    ...
    for example in tqdm.tqdm(examples):
        ...
        example_support = np.argmax(
            trained_model.predict(example_strings),
            axis=1,
        )
