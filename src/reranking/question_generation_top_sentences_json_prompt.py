...
def claim2prompts(example):
    ...
    for question in example["questions"]:
        ...
        for a_text in answer_strings:
            ...
            this_q_claim_str = {"Claim": claim, "Evidence": a_text.strip(), "Question": q_text}
            yield (
                prompt_lookup_str,
                this_q_claim_str,
            )
...
def construct_json_prompt(claim, evidence, similar_qa_pairs):
    prompt = {
        "examples": similar_qa_pairs + [{
            "Claim": claim,
            "Evidence": evidence,
        }],
        # "target example": {
        ...
        #     "Evidence": evidence,
        # },
        "task": "generate the question for last example.",
    }
        ...
        with open(args.top_k_target_knowledge, "r", encoding="utf-8") as json_file:
            for i, line in enumerate(json_file):
                ...
                for sent_i, sentences_urls in enumerate(top_k_sentences_urls):
                    ...
                    prompt = construct_json_prompt(claim, sentence, prompt_docs)
