...
def retrieve_top_k_sentences(query, document, urls, top_k):
    tokenized_docs = [" ".join(nltk.word_tokenize(doc)) for doc in document]
    ...
    # Transform the document and the query to TF-IDF vectors
    sentences_tfidf = vectorizer.transform(tokenized_docs)
    query_tfidf = vectorizer.transform([query])

    # Compute the cosine similarity between the query and each sentence
    scores = cosine_similarity(query_tfidf, sentences_tfidf).flatten()
    top_k_idx = np.argsort(scores)[::-1][:top_k]
