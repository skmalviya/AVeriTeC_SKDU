...
if __name__ == "__main__":
    ...
    # Load the sent model
    model_sent = CrossEncoder('bert_weights/ms-marco-MiniLM-L-6-v2', device="cuda" if torch.cuda.is_available() else "cpu")
        ...
        with open(args.top_k_target_knowledge, "r", encoding="utf-8") as json_file:
            for i, line in enumerate(json_file):
                ...
                for sent_i, sentences_urls in enumerate(top_k_sentences_urls):
                    ...
                    # Compute similarity score between query and all document embeddings
                    len_prompt_corpus = len(prompt_corpus)
                    sent_comb = [(s1,s2) for s1, s2 in zip([prompt_lookup_str]*len_prompt_corpus, prompt_corpus)]
                    prompt_s = model_sent.predict(sent_comb)
